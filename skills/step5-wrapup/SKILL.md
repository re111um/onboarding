...
1. progress.json에서 `notion_kanban_ds`를 읽고, 칸반 DB에서 "참관" 키워드로 카드를 검색한다
2. 찾으면 해당 카드(실서비스 참관) URL을 안내한다: "참관 가이드는 여기 있어요!"
3. 못 찾으면 fallback: `notion-fetch`로 일반 가이드 페이지(`step6_live_audit` ID)를 안내한다
3. 다음 3가지를 당부한다:
   - 노션 가이드를 꼭 확인할 것
   - 참관 대상을 확정할 것
   - **팀 내에 참관 일정 + 근무 스케줄을 꼭 공유**할 것
4. 마무리 멘트 — **이 안내를 마지막으로 클로드를 통한 온보딩은 종료**:
   > "이것으로 클로드를 통한 온보딩이 모두 끝났어요!
   > 실서비스 참관은 노션 가이드를 참고해서 직접 진행해주세요.
...
6. **자동 Slack 알림 발송** — `config/team-leads.json`에서 대상자의 소속팀 정보를 조회하여:
   ...
   - **팀장(C레벨)이 온보딩 대상자인 경우**: 대표({{CEO_NAME}})에게 DM
   - DM 내용:
     > ":tada: *온보딩 완료 알림*\n{닉네임} ({직무}, {팀})의 기본 온보딩이 완료되었습니다.\n실서비스 참관 참여를 독려해주세요!"
   - **HR({{HR_LEAD}})**에게도 항상 온보딩 완료 DM 발송:
     > ":white_check_mark: *온보딩 완료*\n{닉네임} ({직무}, {팀})의 클로드 온보딩이 모두 완료되었습니다. 실서비스 참관은 리더에게 독려 메시지를 보냈어요."
